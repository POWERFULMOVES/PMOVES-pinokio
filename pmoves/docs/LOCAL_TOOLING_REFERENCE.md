...
- Extract worker embeddings: set `EMBEDDING_BACKEND=tensorzero` (default `sentence-transformers`) to route chunk embeddings through the TensorZero gateway, honouring `TENSORZERO_BASE_URL` and `TENSORZERO_EMBED_MODEL` (`embeddinggemma:300m`). Restart `extract-worker` after changes.
